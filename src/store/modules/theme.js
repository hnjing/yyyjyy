...
import {
  changeColor,
  changeSkin
} from '@hnjing/zxzy-admin-plus/es/utils/theme-util';
 ...
 * 需要排除缓存的属性
 */
const CACHE_EXCLUDES = ['collapse', 'compact', 'maximized', 'contentWidth'];

/**
 * 预设皮肤背景
 */
export const PREDEFINE_SKINS = [
  {
    name: 'gradient',
 ...
 */
async function getSkinConfig(config, isId, isCache) {
  if (!config) {
    return config ?? null;
  }
  const skin = { ...config };
 ...
 * @param cfg 默认的皮肤背景
 */
async function getCacheSkinConfig(cfg) {
  const cache = getCacheSetting();
 ...
 */
function changeTheme(color, dark, skinConfig) {
...
export const useThemeStore = defineStore('theme', {
  ...
  actions: {
     ...
     * @param isAll 是否全部释放
     */
    async releaseSkinBgCache(isAll) {
      // 释放全部
    ...
    setFontSize(value, tableSuit = false) {
      this.tableSuit = tableSuit;
      this.fontSize = value;
