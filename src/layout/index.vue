<template>
  ...
  >
    ...
    <template #right>
      <!-- 字体切换 -->
      <layout-tool>
        <SetFontSize :icon-style="{ transform: 'scale(1.15)' }" />
      </layout-tool>
      <!-- 全屏切换 -->
...
<script lang="ts" setup>
  ...
  } from '@hnjing/zxzy-admin-plus/es/ele-pro-layout/types';
  import type { DropdownItem } from '@hnjing/zxzy-admin-plus/es/ele-dropdown/types';
  import {
    MenuFoldOutlined,
    MenuUnfoldOutlined,
    ReloadOutlined,
  ...
  import { useMobileDevice, useMobile } from '@/utils/use-mobile';
  ...
  /** 菜单数据 */
  const { menus, menusSwitch } = storeToRefs(userStore);
  ...
  const handleToHome = () => {
    userStore.setMenusSwitch(false);
    userStore.setMenus(menuList)
    push('/');
  };
