...
<script setup>
  ...
  import { isColorValue } from '@hnjing/zxzy-admin-plus/es/utils/theme-util';
  import {
    CheckOutlined,
    StopOutlined,
    PlusOutlined
  ...
  /** 颜色选择器选中颜色 */
  const colorPickerValue = ref(void 0);

  /** 常用布局 */
  const predefineLayouts = ref([
    {
      name: 'default',
  ...
  const saveUserSkin = (file, isOverlayBg, isDark) => {
    updateValue('skinConfig', async (storeWallpaperFile) => {
      const skin = {
        ...userSkin.value,
  ...
  /** 切换常用布局 */
  const handleLayoutItemClick = (item) => {
